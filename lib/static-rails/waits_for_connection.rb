module StaticRails
  class WaitsForConnection
    def call(site)
      ...
      loop do
        Socket.tcp(site.serve_host, site.serve_port, connect_timeout: 5)
        break
        ...
        if elapsed > timeout
          ...
          unless wait_message_logged
            Rails.logger.info "=> Static site server \"#{site.name}\" is not yet accepting connections on #{site.serve_host}:#{site.serve_port}. Will try to connect for #{timeout} more seconds"
